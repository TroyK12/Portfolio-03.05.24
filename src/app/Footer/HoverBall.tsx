...
export default function HoverBall({ children }: any) {
    ...
    const scale = useTransform(scrollYProgress, [0, 0.35, 0.8, 0.85, 1], [1, 30, 30, 0, 0])
    ...
    return (
        <motion.div ref={ref} className="relative h-screen w-screen">
            <div onMouseMove={handleMouseMove} className="relative w-full flex justify-center ball-hover-effect z-20">
                <motion.div
                    style={{ scale }}
                    className="absolute top-[-20%] md:top-0 w-3 h-3 sm:w-5 sm:h-5 md:w-6 md:h-6 lg:w-[30px] lg:h-[30px] xl:w-11 xl:h-11 rounded-full shadow-2xl overflow-hidden transform-all ease-in-out duration-300">
                ...
                <motion.div
                    style={{ opacity, x }}
                    className="absolute top-[-20vh] w-[75vw] md:w-[50vw] h-[50vh] flex flex-col items-center justify-start md:justify-start gap-1 sm:gap-6 p-10 md:p-0">
                    <div>
                    ...
                    <Link
                        ...
                        target="_blank"
                        className="w-[80%] md:w-[25%] text-center p-3 text-sm lg:text-base border rounded-xl whitespace-nowrap cursor-pointer bg-transparent transform-background duration-500 ease-in-out hover:bg-[#515960]">
                        Source C0de
                    </Link>
                </motion.div>
            </div>
