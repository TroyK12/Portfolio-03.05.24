...
export default function Navbar() {
	...
	return (
		<div className="fixed top-0 w-screen h-36 px-10 z-10">
			<div className="flex flex-row items-center justify-between h-full w-full">
					...
					<Link href="/" className="flex items-center font">
						<Image
							...
							className={`transition-all ease-in-out duration-500 ${
								notOnTop ? 'w-[100px]' : 'w-[70px]'
							}`}
						/>
						<h4
							className={`${
								styles.name
							} tracking-widest text-xl md:text-3xl transition-all ease-in-out duration-500 ${
								notOnTop ? 'opacity-0' : 'opacity-100'
							}`}>
							Troy Kush
						</h4>
					</Link>
				...
				<div className="flex items-center flex-col">
					<button
						...
						<div
							className={`${styles.navBars} h-[3px] rounded-xl w-[40px] ${
								isMenuOpen ? 'translate-y-[15px]' : 'mt-[10px]'
							} ease-in-out duration-1000 transform ${
								isMenuOpen ? 'rotate-45' : 'rotate-0'
							}`}
						/>
						<div
							className={`${styles.navBars} h-[3px] ${
								isMenuOpen
									? 'w-0'
									: isMenuHover
									? 'w-[110%] mt-[10px]'
						...
						<div
							className={`${styles.navBars} h-[3px] rounded-xl w-[40px] ${
								isMenuOpen ? 'translate-y-[10px]' : 'mt-[10px]'
							} ease-in-out duration-1000 transform ${
					...
					<div
						...
						} flex text-lg sm:text-xl font-light bg-[#0b132b1e] backdrop-blur-xl p-10 shadow-xl overflow-hidden transition-all duration-500 ease-linear h-screen w-[85vw] sm:w-[65vw] md:w-[45vw] lg:w-[35vw]`}>
								...
								<li
									className={`relative transition-all ${
										isMenuOpen
											? 'right-0 opacity-1'
											: 'right-[-250px] opacity-0'
									} ease-in-out duration-[2.8s]`}>
									<Link href="tel:2185764997" className={styles.linkHover}>
										(218) 576-4997
									</Link>
								</li>
								<li
									...
									<Link
										href="mailto:troykush@gmail.com"
										className={styles.linkHover}>
										troykush@gmail.com
									</Link>
								</li>
			...
			<div
				className={`w-full h-[1px] bg-[#d6d6d632] transition-all ease-in-out duration-500 ${
					notOnTop ? 'opacity-0' : 'opacity-100'
				}`}
			/>
		</div>
